...
#     1. active-use vs mentioned-use detection (classify_operator_usage),
#     2. metalinguistic_operator_mention relations for words the text talks about,
#     3. corrected conclusion target extraction after fronted therefore/thus/hence,
#     4. list-context propagation so not/maybe/possibly inside a list-or are
#        not double-counted as active scopes,
#     5. relation graph overlay (plot_relation_graph) for span topology.
#
# Carried over from v3:
#     clause-aware tokenization, bounded relation scopes via bounded_end,
#     per-relation confidence, thin bounded_bridge_kernel, relation-energy
#     normalization, run-folder output layout under outputs/v4/runs/{run_id}/.
#
# Dependencies:
#     pip install numpy matplotlib
#
# Run:
#     python logical_surface_model_v4.py
#     python logical_surface_model_v4.py --text "If A then B, but maybe not C."
#     python logical_surface_model_v4.py --file input.txt --no-plots
#
# With uv:
#     uv add numpy matplotlib
#     uv run logical_surface_model_v4.py --no-plots
# =============================================================================
...
def classify_operator_usage(
    ...
    if is_quoted_mention(original_text, first):
        return "mentioned", 0.92, "quoted_operator_word", MENTION_USAGE_DAMPING
...
def infer_logical_relations(tokens: Sequence[Token], events: Sequence[OperatorEvent]) -> List[LogicalRelation]:
    ...
    This is still heuristic, but v4 intentionally avoids v2's main failure mode:
    ...
    for event in events:
        ...
        if event.usage == "mentioned":
            relation = make_relation(
                kind="metalinguistic_operator_mention",
                family=event.family,
                ...
                sentence_index=event.sentence_index,
                clause_index=event.clause_index,
                label=f"mentioned_word:{event.phrase}",
...
def bounded_bridge_kernel(
    ...
    *,
    ...
    v2 used broad interval fields; v4 uses a narrow bridge that fades outside
...
def build_surface(
    ...
    for relation in relations:
        ...
        if relation.kind in {"negation_scope", "uncertainty_scope", "conditional_scope", "list_disjunction", "metalinguistic_operator_mention"}:
            kernel = scope_kernel(
                X,
                Y,
...
def plot_relation_graph(
    ...
    events: Sequence[OperatorEvent],
    relations: Sequence[LogicalRelation],
    title: str = "Logical Relation Graph v4",
    save_path: Optional[Path] = None,
    show: bool = True,
) -> None:
    """
    ...
    *,
    ...
    if show_plots or run_dir is not None:
        plot_surface(
            surface,
            field="total",
            title="Logical Surface of Text v4: Bounded Operators + Relations",
        ...
        plot_heatmap(
            surface,
            field="total",
            title="Total Logical Surface Heatmap v4",
        ...
        plot_heatmap(
            surface,
            field="operator",
            title="Operator-Only Logical Field v4",
        ...
        plot_heatmap(
            surface,
            field="relation_raw",
            title="Raw Relation/Scope Logical Field v4",
        ...
        plot_heatmap(
            surface,
            field="relation",
            title="Energy-Normalized Relation/Scope Logical Field v4",
        ...
        plot_relation_graph(
            ...
            events,
            relations,
            title="Logical Relation Graph v4: Span Topology",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Logical Surface Model v4")
